...
export class TextService {
  ...
  formatHtml(text: string, uppercase: boolean, letterType: any): string {
    const textUC = text.toUpperCase();
    let html = '';
    const parsedText = this.parseText(textUC);
    let charIndex = 0;
  ...
  formatBook(book: Book): void {
    ...
    for (let pageIndex = 0; pageIndex < pages.length; pageIndex++) {
      ...
      if (page.text != undefined) {
        content = this.formatPdfParagraph({ text: page.text, uc: true, html: '' });
      }
